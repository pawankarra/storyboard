...
class File_search_media_class {
	...
	search(query = '', data = []) {
		...
		var settings = {
			title: 'Search',
			comment: 'Powred by <a class="grey" href="http:n3n.io">n3n.io</a>.',
			className: 'wide',
			params: [
			...
			on_load: function (params) {
				...
				for (var i = 0; i < targets.length; i++) {
					targets[i].addEventListener('click', function (event) {
						...
						_this.File_open.file_open_url_handler(data,"new");
					});
				}
			},
			on_finish: function (params) {
				...
				if (_this.cache[params.query] != undefined) {
					...
					//query to service
					var URL = "http://54.186.153.173:9000/n3n/cloud/images/placeholder";
